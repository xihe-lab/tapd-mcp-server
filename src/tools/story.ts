...
export const storyTools: ToolDef[] = [
  // 需求 CRUD
  {
    ...
    inputSchema: z.object({
      ...
      id: z.string().optional().describe('需求ID，支持多ID查询'),
      ...
      category_id: z.string().optional().describe('需求分类'),
      include_sub_category: z.string().optional().describe('包含子分类：0或1'),
      parent_id: z.string().optional().describe('父需求ID'),
      ancestor_id: z.string().optional().describe('祖先需求ID'),
      children_id: z.string().optional().describe('子需求ID'),
      ...
      release_id: z.string().optional().describe('发布计划'),
  ...
  },
  {
    ...
    inputSchema: z.object({
      ...
      workitem_type_id: z.string().optional().describe('工作项类型ID（可选，默认创建需求；传入 TASK 类型 ID 可创建任务。可通过 tapd_get_workitem_types 查询可用类型）'),
    }),
    handler: async (client, params) => {
  ...
  },
  {
    ...
    inputSchema: z.object({
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      id: z.string().optional().describe('需求ID'),
      name: z.string().optional().describe('需求标题'),
      priority: z.string().optional().describe('优先级'),
      priority_label: z.string().optional().describe('优先级标签（推荐）'),
      status: z.string().optional().describe('状态'),
  ...
  },
  {
    ...
    inputSchema: z.object({
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      id: z.string().describe('要复制的需求ID (必填)'),
  ...
  // 需求分类
  {
    ...
    inputSchema: z.object({
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      id: z.string().optional().describe('分类ID，支持多ID查询'),
  ...
  },
  {
    ...
    inputSchema: z.object({
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      id: z.string().optional().describe('分类ID'),
  ...
  },
  {
    ...
    inputSchema: z.object({
      id: z.string().describe('需求ID (必填)'),
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      workitem_type_id: z.string().describe('新的需求类别ID (必填)'),
    }),
    handler: async (client, params) => {
      const workspaceId = params.workspace_id ?? TapdClient.getDefaultWorkspaceId();
      if (!workspaceId) {
  ...
  // 关联关系
  {
    ...
    inputSchema: z.object({
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      story_id: z.string().describe('需求ID (必填)'),
      limit: z.number().optional().describe('返回数量'),
      page: z.number().optional().describe('页码'),
      fields: z.string().optional().describe('返回字段'),
    }),
  ...
  },
  {
    ...
    inputSchema: z.object({
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      story_id: z.string().describe('需求ID (必填)'),
      tcase_id: z.string().optional().describe('测试用例ID'),
      limit: z.number().optional().describe('返回数量'),
      page: z.number().optional().describe('页码'),
      fields: z.string().optional().describe('返回字段'),
    }),
  ...
  },
  {
    ...
    inputSchema: z.object({
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      story_id: z.string().describe('需求ID (必填)'),
      tcase_id: z.string().describe('测试用例ID (必填)'),
    }),
    handler: async (client, params) => {
      const workspaceId = params.workspace_id ?? TapdClient.getDefaultWorkspaceId();
      if (!workspaceId) {
  ...
  // 前后置关系
  {
    ...
    inputSchema: z.object({
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      story_id: z.string().optional().describe('需求ID'),
      predecessor_id: z.string().optional().describe('前置需求ID'),
      successor_id: z.string().optional().describe('后置需求ID'),
  ...
  },
  {
    ...
    inputSchema: z.object({
      id: z.string().describe('需求ID (必填)'),
      workspace_id: z.number().optional().describe('项目ID（可省略，使用默认配置）'),
      parent_id: z.string().describe('新的父需求ID (必填)，设为0取消父子关系'),
    }),
    handler: async (client, params) => {
      const workspaceId = params.workspace_id ?? TapdClient.getDefaultWorkspaceId();
      if (!workspaceId) {
